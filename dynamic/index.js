$(function() {
    ...
    c.load("<div class='bef_comment writecode'>&nbsp;*hi,我的名字叫Euler，是广州一家创业公司的技术总监</div>");
    c.load("<div class='bef_comment writecode'>&nbsp;*我的主要工作是负责公司技术管理体系的建设和维护</div>",2000);
    c.load("<div class='bef_comment writecode'>&nbsp;*以下为对自己的简单介绍</div>");
    ...
    w.load("<p><div class='writecode'>Euler</div>");
    w.load("<div class='writecode'>技术总监</div>");
    w.load("<h1 class='writecode'>联系我</h1>");
    w.load("<ul>");
    w.load("<li class='writecode'>mail:xuanshuangchen@gmail.com</li>");
    w.load("<li class='writecode'>blog:<a href='http://euler-king.github.io' target='_blank'>http://euler-king.github.io/</a></li>");
    w.load("<li class='writecode'>github:<a href='https://github.com/euler-king' target='_blank'>https://github.com/euler-king</a></li>");
    w.load("</ul>");
    w.load("<h1 class='writecode'>技能</h1>");
    w.load("<ul>");
    w.load("<li class='writecode'>Spring Cloud系列框架的实际项目应用。如：eureka，gateway，bus，kafka，config等</li>");
    w.load("<li class='writecode'>熟练使用Linux下工具对系统进行部署和运维。</li>");
    w.load("<li class='writecode'>基于mybatis generator进行二次开发，一键生成Mapper/DAO/Service，抽象出常用的操作方法，支持常用的增删改查，排序，树形结构操作，最大值等方法。已完善支持Spring Cloud的Consumer与Producer，创建一个表便可支持Spring Cloud网关层的访问。</li>");
    w.load("<li class='writecode'>搭建gitlab仓库作为公司的代码管理仓库，进行分支与权限管理。</li>");
    w.load("<li class='writecode'>搭建svn仓库作为公司的文件共享仓库，对项目的文档进行权限管理。</li>");
    w.load("<li class='writecode'>搭建zentao平台用于研发过程管控。</li>");
    w.load("<li class='writecode'>搭建jenkins用于持续集成交付，通过vSphere虚拟化进行远程部署。</li>");
    w.load("<li class='writecode'>后台开发采用SpringCloud+SpringBoot+Mybatis的框架；通过Spring boot减少Spring的配置，启动时也可指定配置文件、配置参数；通过maven进行工程依赖管理；搭建nexus作为后台maven仓库，方便后台的服务化开发。系统开发使用常用的设计模式，递归，泛型，反射等，增加系统的扩展性。集成apiDoc，编写接口文档，方便前后端的对接与接口文档的维护。</li>");
    w.load("<li class='writecode'>编写maven插件，实现组件化开发中jar包配置文件的合并。</li>");
    w.load("<li class='writecode'>使用netty+protobuf；websocket/socket的通讯，实现命令分发的可扩展性。</li>");
    w.load("<li class='writecode'>使用postgresql、mysql、mongodb、sqlite等多种数据库，丰富的数据库设计经验，有丰富的sql优化经验。搭建相关的数据库高可用集群（主从、主备、主主）</li>");
    w.load("<li class='writecode'>使用redis作为缓存，高效的存取数据，对应用的状态进行分离，并可搭建redis分布式集群，提高系统的可用性。</li>");
    w.load("<li class='writecode'>使用fastdfs进行分布式文件存储，在官方提供的连接方案之后进行集成，实现common-pool2连接池的方式保存连接信息，支持高并发下的数据存取，同时利用fastdfs的特性进行文件的读写分离与文件的归档和备份。</li>");
    w.load("<li class='writecode'>使用nginx进行反向代理、负载均衡、高可用、宕机容错等方案。</li>");
    w.load("<li class='writecode'>使用activemq进行消息的异步处理与分发， 搭建kafka集群，支撑spring cloud config服务。</li>");
    w.load("<li class='writecode'>使用vue，基于vue-cli3构建起管理后台项目与移动开发项目。涉及到vue-router、vuex、axios、iview、element-ui等。并通过verdaccio建立npm私有仓库，构建前端组件化开发流程。</li>");
    w.load("<li class='writecode'>移动端通过cordova，搭建基础工程，编写ANDROID与IOS一键打包脚本，对icon、screen的更换、不同项目插件的增减提供可插拔。</li>");
    w.load("<li class='writecode'>项目中有C#、ICE Zero、ibmmq、ftp的开发使用。</li>");
    w.load("<li class='writecode'>根据项目需要可编写相关架构文档，设计文档，网络拓扑，硬件拓扑图，技术架构等。 </li>");
    w.load("</ul>");
    w.load("<h1 class='writecode'>项目</h1>");
    // w.load("<div class='writecode'></div>");
    w.load("<ul>");
    w.load("<li class='writecode'><a href='http://www.roywise.cn' target='_blank'>www.roywise.cn</a> | 主要业务中的全部项目开发</li>");
    w.load("</ul>");
    w.load("<h1 class='writecode'>技能描述</h1>");
    w.load("<div class='writecode'><img src=\"/assets/images/struct.png\" style='margin-top: 10px;width: 80%;' /></div>");
    c.load("<p><div class='comment writecode'>/**</div>");
    ...
    c.load("<div class='comment writecode'>&nbsp;*QQ/微信:471023334/xuanshuangchen</div>");
